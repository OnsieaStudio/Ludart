...
public class GLFWWindow implements IWindowSystem
{
	private @Getter long handle;

	@Override
	public void initialization(IWindowSettings settingsIn)
	{
		...
		glfwWindowHint(GLFW_REFRESH_RATE, settingsIn.refreshRate());

		handle = glfwCreateWindow(settingsIn.width(), settingsIn.height(), settingsIn.title(), NULL, NULL);
		if (handle == NULL)
		{
